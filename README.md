...
- 📱 **响应式**:文章卡片网格,移动端自适应
- 📰 **封面图文章卡**:横向 media-object 卡片(左封面 + 右内容),无图自动回退纯文字卡,封面懒加载
- 👁️ **浏览数统计**:零依赖 post_meta 计数 + 防刷新(transient),卡片与单篇页显示
- 🎛️ **后台可配置**:`外观 → 自定义` 里开关封面图/浏览数/评论数/字数/标签、调摘要行数、作者卡头像来源
- 🧩 **古腾堡友好**:`theme.json` 配色 / 字体对齐,兼容各类块(block)
- ⚙️ **主色可调**:后台 `自定义` 里用滑块调色相(0–360)
- ☁️ **云存储 Offload**:媒体自动推送对象存储(阿里 OSS / 腾讯 COS / 七牛 / 又拍 / 华为 OBS / AWS S3·MinIO)+ 前端 URL 替换,后台「云存储」配置,内置连接测试
