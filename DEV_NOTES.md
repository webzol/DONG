...
## v6.3.3(2026-08-06)· 资源页标题从背景 Banner 独立

### 背景
- TD 反馈 `/resources` 的背景表现不对，并明确“实用好物集”是单独的标题展示，不应作为带颜色 / 图片背景的 Banner 内容。

### 改动
- **`inc/resources.php`**：`onedong_resource_banner()` 不再输出 `.resource-banner`、背景模式、动效和内容卡片；改为语义化 `.resource-page-heading`，只承载后台配置的标题与副标题。
- **`assets/css/resources.css`**：新增独立标题样式，与筛选栏 / 资源卡使用同一 `--site-width` 和水平 gutter；透明无背景，标题 / 副标题分别使用 `--text` / `--text-muted`，自动兼容浅色、深色和 zhipu 皮肤；移动端缩小字号和上下间距。资源页外层、筛选按钮和资源卡背景统一改为 `var(--bg)`，继承网站整体背景，仅靠边框 / 阴影 / 激活态区分层级，不再出现独立蓝色内容底。
- 资源卡、筛选器、底部居中伸缩“提交资源”入口及 `/resources/submit/` 提交流程均保持不变。
- 版本 `6.3.2→6.3.3-ProMax`，同步更新 `style.css` 与 `ONEDONG_VERSION`。
...
## v6.3.6(2026-08-07)· 资源页主题色标题带 + 白色卡片

### 背景
- TD 要求 `/resources` 主标题“实用好物集”默认使用主题色背景并居中展示，同时每个资源卡片固定为白色。

### 改动
- **`assets/css/resources.css`**：标题区域改为全宽 `var(--primary)` 背景，主标题与副标题使用白色系文字并居中；移动端使用对称紧凑内边距。
- 资源卡背景固定为白色，并在卡片作用域内定义深色正文、次级文字、边框和强调色，避免深色主题的浅色文字落到白色卡片上；zhipu 皮肤继续使用其 `#0066ff` 强调色。
- 卡片布局、圆角、阴影、hover 展开、筛选与空状态逻辑保持不变。版本 `6.3.5→6.3.6`，同步更新 `style.css` 与 `ONEDONG_VERSION`。
...
## v6.3.8(2026-08-07)· 标题横幅内容区对齐 + 显示开关

### 背景
- TD 反馈 `/resources` 蓝色标题横幅与下方筛选器 / 卡片左右边缘未对齐，并要求标题为空时不显示蓝色背景，同时可在后台主动控制标题区域显隐。

### 改动
- **`inc/resources.php`**：新增默认开启的 `banner_enabled` 设置与“显示标题区域”复选框；保存值统一为 `'1'/'0'`。开关关闭或主标题为空 / 纯空白时，前台在输出 HTML 前直接返回，标题、副标题、背景和高度占位均不渲染。
- **`assets/css/resources.css`**：资源页定义共享 `--resources-gutter`，标题横幅宽度和 `.resources-main` 水平 padding 共用同一变量，使横幅与筛选器、首尾卡片在桌面和移动端保持同一左右边缘。
- v6.3.7 的 Banner 高度设置联动与移动端自动收缩保持不变。版本 `6.3.7→6.3.8`，同步更新 `style.css` 与 `ONEDONG_VERSION`。
